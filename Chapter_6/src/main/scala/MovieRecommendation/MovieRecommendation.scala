...
object MovieRecommendation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val new_user_ratings = Seq(
      (new_user_ID, 260, 4.5), // Star Wars (1977)
      (new_user_ID, 1, 3.25), //Toy Story (1995)
      (new_user_ID, 16, 3.60), // Casino (1995)
      (new_user_ID, 25, 4.10), // Leaving Las Vegas (1995)
      (new_user_ID, 32, 4.00), // Twelve Monkeys (a.k.a. 12 Monkeys) (1995)
      (new_user_ID, 335, 1.50), // Flintstones, The (1994)
      (new_user_ID, 379, 1.50), // Timecop (1994)
      (new_user_ID, 296, 3.25), // Pulp Fiction (1994)
      (new_user_ID, 858, 5.00), // Godfather, The (1972)
      (new_user_ID, 50, 4.20) // Usual Suspects, The (1995)
      )

    val new_user_ratings_RDD = spark.sparkContext.parallelize(new_user_ratings)
    val new_user_ratings_DF = spark.createDataFrame(new_user_ratings_RDD).toDF("userID", "movieID", "Rating")
    ...
    val complete_data_with_new_ratings_RDD = ratingsRDD.union(newRatingsRDD)
    ...
    println("Rating:(UserID, MovieID, Rating)")
    println("----------------------------------")
    val newPrediction = newModel.recommendProducts(669, 6)
    for (rating <- topRecsForUser) {
      println(rating.toString())
    }
    println("----------------------------------")
